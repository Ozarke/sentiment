...
function roll_to_dye(obj,d6,bonus) {
    ...
    var outstr = '&{template:custom}{{color='+ background_color + '}}{{title=[Dye](' + header_url + ')}} ';
    var roll_val = 0;
    var roll_level = 0;
    var total_val = 0;
    var attribute = 0;
        ...
        if(getAttrByName(obj.id, colors[i]+'_enabled','Current') === 'true'){
            ...
            if(getAttrByName(obj.id,colors[i] + '_wounded','Current') === 'true'){
                ...
                if(getAttrByName(obj.id,colors[i] + '_lockedout','Current') === 'true'){              
                    outstr += '= ``Lockedout``[Unlock](!wind set-attribute-state ' + obj.id + ' ' + colors[i] + ' ' +'_lockedout'+ ' ' +'false)}}';
                    ...
                    if(colors[i] === getAttrByName(obj.id,"swing_color","Current"))
                        ...
                        outstr +='   [Keep](!wind set-swing ' + obj.id +' ' + colors[i] + ' '+ roll_val+ ' true)}}';
                }
            }
        }
    }
